...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="metadata-dcat-ap">
	...
	<xsl:template mode="dcat-ap" match="dct:anyCHOICE_ELEMENT0">
		...
		<xsl:apply-templates mode="elementEP" select="dct:*|geonet:child[string(@prefix)='dc']">
      <xsl:with-param name="schema" select="$schema"/>
      <xsl:with-param name="edit"   select="$edit"/>
    </xsl:apply-templates>
      ...
      <xsl:with-param name="content">
        ...
        <xsl:choose>
          <xsl:when test="$edit">
            ...
            <!-- Remove button -->
            <xsl:if test="$button">
              <xsl:apply-templates mode="dcatapFileRemove" select=".">
            ...
            <xsl:call-template name="simpleElementGui">
              ...
              <xsl:with-param name="text">
                ...
                <!-- Field checked in metadata-editor.js (doFileUploadSubmit), to execute custom code for DCAT-AP metadata editor -->
  ...
  <xsl:template mode="dcatapFileRemove" match="*">
